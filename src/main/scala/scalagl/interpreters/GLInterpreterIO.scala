...
object GLInterpreterIO extends WebGL[IO] {
  ...
  private lazy val context =
    can.getContext("webgl").asInstanceOf[dom.raw.WebGLRenderingContext]
  ...
  def createShader(shader: ShaderType): IO[WebGLShader] =
    gl(_.createShader(shader match {
      case Vertex   => VERTEX_SHADER
      case Fragment => FRAGMENT_SHADER
    }))

  def shaderSource(shader: WebGLShader, src: String): IO[Unit] =
    gl(_.shaderSource(shader, src))
  ...
  def getShaderInfoLog(shader: WebGLShader): IO[String] =
    gl(_.getShaderInfoLog(shader))

  def createProgram(): IO[WebGLProgram] = gl(_.createProgram())

  def attachShader(program: WebGLProgram, shader: WebGLShader): IO[Unit] =
    gl(_.attachShader(program, shader))
  ...
  def getProgramInfoLog(program: WebGLProgram): IO[String] =
    gl(_.getProgramInfoLog(program))
  ...
  def bindBuffer(target: Int, buffer: WebGLBuffer): IO[Unit] =
    gl(_.bindBuffer(target, buffer))

  def bufferData(target: Int, data: Float32Array, usage: Int): IO[Unit] =
    gl(_.bufferData(target, data, usage))
